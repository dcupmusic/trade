...
import torch
import joblib
from lstm_v1_Haas import LSTMClassifier

app = Flask(__name__)

model = LSTMClassifier(3, 32, 2, 3)
...
def predict():
    try:
        ...
        price_scaled = (price - mean[1]) / scale[1]
        rsi_scaled = (rsi - mean[2]) / scale[2]

        # Prepare the input data as a scaled tensor
        input_data = torch.tensor([[timestamp_scaled, price_scaled, rsi_scaled]], dtype=torch.float)
